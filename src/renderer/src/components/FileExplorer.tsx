...
// Custom-Symbol für ein Audio-Dateisymbol (Blatt mit gefalteter Ecke und großem, farbigem Notenschlüssel)
const AudioFileIcon = ({ size = 16, className = "text-blue-400" }: { size?: number; className?: string }) => (
  <svg 
  ...
  >
    {/* Page Sheet */}
    <path 
      d="M 12 12 C 12 9.8 13.8 8 16 8 L 52 8 L 78 34 L 78 78 C 78 80.2 76.2 82 74 82 L 16 82 C 13.8 82 12 80.2 12 78 Z" 
      fill="#ffffff" 
      stroke="#1e293b" 
      strokeWidth="5" 
...
export function FileExplorer() {
  ...
  const navigateTo = (entry: FileEntry) => {
    if (entry.isDirectory) {
      ...
      if (entry.name.match(/\.(mp3|wav|ogg|m4a|flac|wma|m4r|aiff|opus|alac)$/i)) {
  ...
  const getFileIcon = (name: string) => {
    ...
    if (['mp3', 'wav', 'ogg', 'm4a', 'flac', 'wma', 'm4r', 'aiff', 'opus', 'alac'].includes(ext)) {
      return <AudioFileIcon size={16} className={getAudioColorClass(ext)} />
    }
        ...
        >
          {filteredFiles.map((file, idx) => (
            ...
            >
              ...
              {!file.isDirectory && file.name.match(/\.(mp3|wav|ogg|m4a|flac|wma|m4r|aiff|opus|alac)$/i) && (
      ...
      {contextMenu && (
        ...
        >
          ...
          {(contextMenu.type === 'file' || contextMenu.type === 'directory') && contextMenu.file && (
            <>
              {contextMenu.type === 'file' && contextMenu.file.name.match(/\.(mp3|wav|ogg|m4a|flac|wma|m4r|aiff|opus|alac)$/i) && (
                <>
                  <button
                    onClick={() => {
            ...
            <>
              ...
              {currentPath && currentPath !== 'computer' && (
                <>
                  ...
                  <button
                    onClick={() => {
                      // Aktuellen Ordner direkt im System-Dateiexplorer öffnen
