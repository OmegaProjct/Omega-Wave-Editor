...
export function AudioRecordingModal({
  ...
  useEffect(() => {
    ...
    const resizeToFit = () => {
      ...
      if (contentEl) {
        ...
        console.log(`[AudioRecordingModal] Dynamic content sizing: title(0) + content(${contentHeight}) + pad(16) = ${totalHeight}px`);
        try {
          window.api.resizeWindow(600, Math.max(350, Math.min(800, totalHeight)));
        } catch (e) {
  ...
  const [savedInfo, setSavedInfo] = useState<{ filePath: string; durationSec: number } | null>(null);
  ...
  const handleStop = async () => {
    ...
    try {
      ...
      if (saveRes && saveRes.success) {
        setSavedInfo({ filePath: finalPath, durationSec: result.durationSec });
        ...
        if (isPopout) {
          ...
          // Let the storage event propagate
          setTimeout(() => {
  ...
  return (
    <div className={isPopout ? "h-screen w-screen bg-[#282b30] text-[#d1d5db] font-sans flex flex-col overflow-hidden select-none" : "fixed inset-0 z-[2000] flex items-center justify-center bg-black/60 backdrop-blur-sm"}>
      <div className={isPopout ? "flex-1 flex flex-col bg-[#282b30] w-full h-full" : "w-[540px] bg-[#282b30] border border-[#3b3e45] text-[#d1d5db] font-sans shadow-2xl flex flex-col rounded-lg overflow-hidden select-none"}>
        ...
        {/* Content */}
        {recordingFinished && savedInfo ? (
          <div id="audio-recording-modal-content" className="p-6 flex flex-col items-center justify-center text-center gap-5 text-xs select-none">
            <div className="w-16 h-16 rounded-full bg-[#10b981]/15 border border-[#10b981]/30 flex items-center justify-center text-[#10b981] shadow-[0_0_20px_rgba(16,185,129,0.15)] mt-2">
              <Check size={32} className="stroke-[2.5]" />
            </div>
            
            <div className="flex flex-col gap-1.5 mt-1">
              <h2 className="text-base font-bold text-white tracking-wide">{t('recording.success_title', { defaultValue: 'Aufnahme erfolgreich gespeichert!' })}</h2>
              <p className="text-gray-400 max-w-sm text-[11px] leading-relaxed">
                {t('recording.success_desc', { defaultValue: 'Die Audiodaten wurden erfolgreich in eine verlustfreie 16-Bit PCM WAV-Datei konvertiert und importiert.' })}
              </p>
            </div>

            <div className="w-full bg-[#1a1d21]/60 border border-[#3b3e45] rounded-xl p-4 flex flex-col gap-3 font-mono text-[11px] text-left text-gray-300">
              <div className="flex justify-between items-center border-b border-[#3b3e45]/30 pb-2">
                <span className="text-gray-400">{t('recording.saved_filename', { defaultValue: 'Dateiname:' })}</span>
                <span className="text-[#4da3ff] font-bold">{savedInfo.filePath.split(/[\\/]/).pop()}</span>
              </div>
              <div className="flex justify-between items-center border-b border-[#3b3e45]/30 pb-2">
                <span className="text-gray-400">{t('recording.duration', { defaultValue: 'Länge:' })}</span>
                <span className="text-white font-bold">{savedInfo.durationSec.toFixed(2)}s</span>
              </div>
              <div className="flex flex-col gap-1">
                <span className="text-gray-400">{t('recording.saved_path', { defaultValue: 'Speicherort:' })}</span>
                <span className="text-gray-500 break-all select-all leading-normal text-[10px]">{savedInfo.filePath}</span>
              </div>
            </div>
            ...
            <div className="flex flex-col gap-2 w-full mt-2">
              <button
                ...
                className="w-full h-10 bg-[#10b981] hover:bg-[#059669] text-white font-bold rounded-xl active:scale-[0.98] transition-all text-xs flex items-center justify-center gap-2 shadow-lg shadow-emerald-950/20 cursor-pointer"
              >
                <span>{t('recording.finish', { defaultValue: 'Fertigstellen' })}</span>
              </button>
              ...
              <button
                ...
                className="w-full h-8 bg-transparent hover:bg-white/5 text-gray-400 hover:text-white font-semibold rounded-xl transition-colors text-xs flex items-center justify-center gap-1.5 cursor-pointer"
              >
                <span>{t('recording.record_another', { defaultValue: 'Weitere Aufnahme starten' })}</span>
              </button>
            </div>
          </div>
        ) : (
          <div id="audio-recording-modal-content" className="p-4 flex flex-col gap-4 text-xs">
            ...
            <div className="flex justify-between items-center bg-[#1a1d21]/30 border border-[#3b3e45]/25 rounded-xl p-4 mt-1.5 shadow-inner">
              {/* Info */}
              <div className="flex flex-col gap-1.5 pr-2">
                <span className="text-[#4da3ff] font-semibold text-xs">{t('recording.info', { defaultValue: 'Aufnahmeinformationen:' })}</span>
                <div className="flex flex-col gap-1 font-mono text-[11px] text-gray-300">
              ...
              <div className="flex items-center gap-6 pl-2 border-l border-[#3b3e45]/40">
                {/* DAW Sync coupling */}
                <div className="flex flex-col gap-1.5 select-none">
                  <span className="text-[#4da3ff] font-semibold block text-center text-xs">{t('recording.daw_sync', { defaultValue: 'DAW-Sync:' })}</span>
                  <label className="flex items-center gap-2 cursor-pointer text-[#d1d5db] py-0.5 hover:text-white transition-colors" title="Startet bei Klick auf Aufnahme automatisch die DAW-Wiedergabe synchron mit.">
                    <input
                      type="checkbox"
                    ...
                    />
                    <span className="font-semibold text-xs">{t('recording.sync_playback', { defaultValue: 'DAW mitstarten' })}</span>
                  </label>
                </div>

                <div className="flex flex-col gap-1">
                  <span className="text-[#4da3ff] font-semibold text-center w-full block text-xs">{t('recording.recording_label', { defaultValue: 'Aufnahme:' })}</span>
                  <div className="bg-[#1a1d21] border border-[#3b3e45] rounded-xl px-4 py-1.5 flex gap-4 items-center justify-center w-26 h-9.5 shadow-inner">
                    {/* Stop button */}
                    <button
                      onClick={handleStop}
                      disabled={!isRecording}
                ...
                <button
                  onClick={isRecording ? handleStop : (onClose || (() => window.close()))}
                  className="px-6 h-9.5 bg-[#4a4d52] hover:bg-[#5c5f66] text-[#d1d5db] hover:text-white font-bold rounded-xl border border-[#3b3e45] active:scale-95 transition-all text-xs flex items-center justify-center shadow-md cursor-pointer mt-3"
                >
